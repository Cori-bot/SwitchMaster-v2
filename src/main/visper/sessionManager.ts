...
export class VisperSessionManager {
    private static getSessionsFile() {
        return path.join(path.dirname(getPaths().ACCOUNTS_FILE), "visper_sessions.json");
    }
    ...
    static async saveSession(authData: VisperAuthSession): Promise<VisperSavedSession> {
        try {
    ...
    static async restoreSession(puuid: string): Promise<boolean> {
        try {
            ...
            devLog(`[SessionManager] Restoring ${cookies.length} cookies for ${target.gameName}...`);
