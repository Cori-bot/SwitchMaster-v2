...
export class RiotWebviewAuth {
    ...
    static async login(parentWindow: BrowserWindow | null = null, silent: boolean = false, forceNew: boolean = false): Promise<VisperAuthSession | null> {
        return new Promise((resolve) => {
            devLog(`[VisperAuth] Creating login window (silent: ${silent}, forceNew: ${forceNew})...`);
            ...
            loginWin.webContents.on("did-fail-load", (_e, errorCode, errorDescription, validatedURL) => {
                devError(`[VisperAuth] Failed to load ${validatedURL}: ${errorDescription} (${errorCode})`);
    ...
    private static async finishAuth(accessToken: string, idToken?: string): Promise<VisperAuthSession | null> {
        try {
            ...
            try {
                ...
                devLog("[VisperAuth] Loadout Data received:", JSON.stringify(loadoutData));
