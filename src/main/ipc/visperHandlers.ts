import {
  RiotWebviewAuth,
  VisperAuthSession,
} from "../valorant-api/riotWebviewAuth";
...
export function registerVisperHandlers(getWin: () => BrowserWindow | null) {
  safeHandle(
    "start-visper-login",
    async (_event, silent: boolean = false, forceNew: boolean = false) => {
      devLog(
        "[IPC] start-visper-login triggered, silent:",
        silent,
        "forceNew:",
        forceNew,
      );
      ...
      devLog(
        "[VisperAuth] Starting login with parent win:",
        win ? "Found" : "None",
      );
      const session = await RiotWebviewAuth.login(
        win || undefined,
        silent,
        forceNew,
      );
  ...
  safeHandle(
    ...
    async (_event, session: VisperAuthSession, partyId: string) => {
  ...
  safeHandle(
    "visper-join-code",
    async (_event, session: VisperAuthSession, code: string) => {
  ...
  safeHandle(
    "visper-get-friends",
    async (_event, session: VisperAuthSession) => {
