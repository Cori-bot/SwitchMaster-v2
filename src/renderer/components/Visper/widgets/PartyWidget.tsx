...
const PartyWidget: React.FC<PartyWidgetProps> = ({ session }) => {
        ...
        <div className="space-y-4">
          <div>
            ...
            <p className="text-gray-400 text-sm max-w-md mx-auto font-medium leading-relaxed mt-2">
              Veuillez lancer{" "}
              <span className="text-white font-bold italic">VALORANT</span> avec
              le compte{" "}
              <span className="text-blue-400 font-bold">
                {session.gameName}#{session.tagLine}
              </span>{" "}
              pour synchroniser{" "}
              <span className="text-white font-bold">VISPER</span>.
            </p>
          </div>
          ...
          <div className="inline-block bg-black/40 border border-white/5 rounded-lg px-4 py-2">
            <p className="text-[10px] text-gray-500 font-bold uppercase tracking-widest">
              ...
              <span className="text-white text-sm">{retryCountdown}</span>
            </p>
          </div>
        </div>
      </div>
  ...
  const currentQueue = QUEUES.find((q) => q.id === party.queueId) || {
    id: party.queueId,
    label: "Inconnu",
  };
  ...
  const podsLabel =
    selectedPods.length > 0
      ? selectedPods.length === 1
        ? GAME_PODS[selectedPods[0]] || "Serveur"
        : `${selectedPods.length} Serveurs`
      : "Automatique";
      ...
      <div className="flex-1 flex flex-col gap-4 h-full">
        ...
        <div className="bg-[#1a1a1a] border border-white/10 rounded-2xl p-3 flex items-center justify-between shadow-xl shrink-0">
          <div className="flex items-center gap-4">
            ...
            <div className="relative z-50">
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  className={`text-white/50 transition-transform ${showQueueSelect ? "rotate-180" : ""}`}
                />
              </button>
              <AnimatePresence>
                {showQueueSelect && (
            ...
            <div className="relative z-40">
              ...
              >
                ...
                <ChevronDown
                  size={14}
                  className={`text-white/50 transition-transform ${showPodSelect ? "rotate-180" : ""}`}
                />
              </button>
              <AnimatePresence>
                {showPodSelect && (
                      ...
                      return (
                        ...
                        >
                          <div className="flex items-center gap-2">
                            ...
                            {ping !== undefined && (
                              <span
                                className={`text-[10px] font-mono ${pingColor}`}
                              >
                                {ping}ms
                              </span>
                            )}
                          ...
                          <div
                            className={`w-3 h-3 rounded border flex items-center justify-center ${isSelected ? "bg-blue-600 border-blue-600" : "border-white/20 bg-black/40"}`}
                          >
                            {isSelected && (
          ...
          <div className="flex items-center gap-3">
            <form
              ...
              className="flex items-center gap-2 bg-black/20 rounded-lg p-1 border border-white/5 focus-within:border-white/20 transition-colors"
            >
              <input
                type="text"
                placeholder="CODE..."
              ...
              <button
                ...
                className="p-1 hover:bg-white/10 rounded text-blue-400 disabled:opacity-20 disabled:cursor-not-allowed"
              >
                <ArrowRight size={14} />
              </button>
            </form>
            ...
            {isLeader && (
              <button
                onClick={() =>
                  actions.toggleOpen(party.accessibility === "CLOSED")
                }
            ...
            <button
              onClick={() => actions.leaveParty()}
              className="p-2 hover:bg-red-500/10 text-gray-500 hover:text-red-500 rounded-xl"
              title="Quitter"
            >
              <LogOut size={18} />
            </button>
        ...
        <div className="flex-1 flex gap-2 w-full overflow-x-auto overflow-y-hidden custom-scrollbar pb-2">
            ...
            return (
              <div
                ...
                className={`relative flex-1 min-w-[180px] max-w-[240px] h-full rounded-xl overflow-hidden border transition-all duration-300 group ${
                  isMe
                    ? "cursor-pointer ring-1 ring-white/20 hover:ring-white/40"
                    : ""
                ...
                <div className="absolute inset-0 z-10 flex flex-col justify-between p-4">
                  ...
                  <div className="flex flex-col items-center gap-2">
                    ...
                    <div className="text-center w-full">
                      <div className="text-lg font-black text-white uppercase tracking-tight truncate drop-shadow-md">
                        {member.name}
                      </div>
                      <div className="text-[10px] text-gray-300 font-mono bg-black/40 px-2 py-0.5 rounded inline-block mt-1">
                        #{member.tag}
                      </div>
                    </div>

                    <div
                      className={`mt-4 w-full py-3 rounded-xl font-black uppercase tracking-[0.2em] text-[10px] text-center backdrop-blur-md transition-all ${
                        member.isReady
          ...
          {Array.from({ length: 5 - party.members.length }).map((_, i) => (
            <div
              key={`empty-${i}`}
              className="flex-1 min-w-[180px] max-w-[240px] h-full rounded-xl border border-white/5 bg-[#151515]/50 flex flex-col items-center justify-center gap-4 group"
            >
          ...
          <div className="flex items-center gap-3 pl-2">
            {party.inviteCode ? (
              <div className="flex items-center gap-3 bg-blue-900/20 border border-blue-500/20 px-4 py-2 rounded-xl">
                ...
                <span className="text-sm font-mono font-bold text-white tracking-[0.15em] select-all">
                  {party.inviteCode}
                </span>
          ...
          <div className="w-64">
            {isQueueing ? (
              ...
              <button
                ...
                disabled={
                  loading || !isLeader || party.members.some((m) => !m.isReady)
                }
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="text-[9px] text-gray-500 truncate">
                    {friend.otherGame
                      ? "Sur League"
                      : friend.note ||
                        (friend.status === "chat" ? "En ligne" : friend.status)}
                  </div>
                </div>
                <button
                  onClick={() =>
                    actions.inviteByName(friend.gameName, friend.tagLine)
                  }
